...
class DiscountPage(BasePage):
    ...
    def wait_check_page(self, timeout=10000):
        ...
        try:
            self.driver \
                .wait_for_element_by_name('立即购买', timeout=timeout)
            return True
    ...
    def checkbox_1(self):
        """半年卡 选择框"""
        ClickBounds().click_bounds(50, 810)
        time.sleep(3)

    @teststep
    def checkbox_2(self):
        """年卡卡 选择框"""
        ClickBounds().click_bounds(50, 1150)
        time.sleep(3)
    ...
    def card_type(self):
        """所选卡片的类型"""
        ele = self.driver.\
            element_by_xpath('//android.webkit.WebView[1]/android.view.View[1]/android.view.View[2]')
    ...
    @teststeps
    def wait_check_buycard_page(self, timeout=10000):
        """以title：“购买学习卡”的text为依据"""
        try:
            self.driver \
                .wait_for_element_by_name('购买学习卡', timeout=timeout)
            return True
        except WebDriverException:
            return False
    ...
    def wait_check_finish_page(self, timeout=10000):
        ...
        try:
            self.driver \
                .wait_for_element_by_name('购买情况', timeout=timeout)
            return True
        except WebDriverException:
            return False
